...
(function () {
  ...
  function openSiteSettingsHelp() {
    ...
    if (isYandex()) {
      setStatus(
        'Яндекс: ≡ / ⋮ → О сайте → Уведомления от этого сайта. Затем вернитесь и «Проверить снова».',
        'bad',
      );
      return;
    }
  ...
  function renderStep() {
    ...
    if (id === 'site') {
      ...
      if (isIos()) {
        ...
        lead.textContent =
          'Нужно разрешить уведомления для agronizer.ru на этой вкладке. Списка Заблокированы может не быть.';
        steps.innerHTML =
          '<li>≡ / ⋮ → <b>О сайте</b> → <b>Уведомления от этого сайта</b></li>' +
          '<li>Если «О сайте» нет — откройте agronizer.ru как вкладку, не с ярлыка</li>' +
          '<li>Вернитесь сюда и нажмите «Проверить»</li>';
        if (openBrowserBtn) openBrowserBtn.hidden = true;
      } else {
  ...
  function onCheckAgain() {
    ...
    setStatus(
      isYandex()
        ? 'Ещё запрещено. ≡ / ⋮ → О сайте → Уведомления от этого сайта, затем «Проверить».'
