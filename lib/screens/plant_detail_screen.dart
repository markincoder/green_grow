...
class PlantDetailScreen extends StatelessWidget {
  ...
  Future<void> _start(BuildContext context) async {
    final startedAt = await showStartDateSheet(context: context, plant: plant);
    if (startedAt == null || !context.mounted) return;

    await store.startPlant(plantId: plant.id, startedAt: startedAt);
    if (!context.mounted) return;

    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(
          '${plant.name} · старт ${formatStartDate(startedAt)} — в Моей зелени',
        ),
              ...
              Wrap(
                ...
                children: [
                  _InfoChip(
                    ...
                    label: '${plant.seedGrams} г на лоток 13×18',
                  ),
                  ...
                  _InfoChip(
                    icon: Icons.schedule,
                    label: 'Цикл ${plant.cycleDaysLabel}',
                  ),
